...
async function download(url_input) {
    ...
    fetch('/download_twitter/' + url, {
      ...
      .then(result => {
        ...
        thumbnail_div.innerHTML = '<img src="' + result.thumbnail + '"  height="100px" width="70px" style="border-radius: 5px">'
        video_title.innerHTML = result.tweet_text + '...'
...
function generate_link(url) {
    console.log(url)
    fetch('/generate_twitter_link/' + url, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(result => {
    ...
    window.location='http://127.0.0.1:8000/generate_twitter_link/' + url 

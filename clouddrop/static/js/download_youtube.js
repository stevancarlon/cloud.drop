...
function generate_link_720p(url) {
    ...
    fetch('/generate_youtube_link/' + url, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(result => {
...
function generate_link_360p(url) {
    console.log('Debug: ', url)
    fetch('/generate_youtube_link/' + url, {
        method: 'GET'
    })

    window.location='http://127.0.0.1:8000/generate_youtube_link/' + url + '_res_360p'
